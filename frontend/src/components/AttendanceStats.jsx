...
export function AttendanceCircle({ percentage, size = 'md' }) {
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      <svg width={dim} height={dim} className="-rotate-90">
        <circle cx={dim / 2} cy={dim / 2} r={r} fill="none" stroke="#e5e7eb" strokeWidth="8" />
        <circle cx={dim / 2} cy={dim / 2} r={r} fill="none" stroke={color} strokeWidth="8"
          strokeDasharray={circumference} strokeDashoffset={offset}
          strokeLinecap="round" style={{ transition: 'stroke-dashoffset 0.8s ease' }} />
      </svg>
      <div className="absolute text-center">
        <span className={`font-bold ${size === 'lg' ? 'text-2xl' : 'text-base'}`} style={{ color }}>{percentage}%</span>
      </div>
    </div>
  );
}
...
export function EligibilityBadge({ percentage }) {
